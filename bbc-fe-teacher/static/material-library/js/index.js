      ...
      */
      ...
      _showDetail: function (item) {
        ...
        if (item.coverUrl && item.resourceFileUrl) {
          ...
          let flashvars = 
            "video=" + this.itemInfo.resourceFileUrl  + 
            "&thumbnail=/" + this.itemInfo.coverUrl + 
            "&port=80"
          // video的整个数据
          let video = 
            '<embed type="application/x-shockwave-flash" src="/static/res/' 
            + config.playerFileName +
            '.swf" width="100%" height="100%" id="f" name="f" flashvars="' 
            + flashvars + 
