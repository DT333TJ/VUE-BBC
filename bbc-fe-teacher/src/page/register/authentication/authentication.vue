...
<script type="text/ecmascript-6">
  ...
  import { validataPhone , validateCodeLength} from 'lib/util'
  export default {
    name: 'Authentication',
    data: function() {
      return {
    ...
    methods: {
      ...
      _checkClick: function() {
        ...
        // 格式校验成功
        if (phoneValidate && codeValidate) {
          requestCheckCode(this.identityForm)
            .then( response => {
              let code = response.code
              switch (code) {
                case 200:
                  let queryObj = {
                    tel: this.identityForm.tel,
                    code: this.identityForm.code
                  }
                  this.identityText = ''
                  this.errorFlag = false
                  this.$router.replace({name: 'set-password', query: queryObj})
                  break
                case 204:
