...
<template>
  <content-box>
    ...
    <Modal
      ...
      <a 
        href="http://123.206.87.247/download/ViewShareBBC幼儿课堂.exe" 
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      _togglePreparePlan: function() {
        requestTogglePreparePlan(this.curCourse.courseId)
          ...
          .catch(err => {
            // console.log('requestToggleTeachingCase FAIL', err)
          })
      },
      ...
      _tabItemClick: function(tabItem) {
        ...
        setTimeout(() => this.loadingVisible = false, 800)
      },
      ...
      _finishPrepareCourse: function(courseId) {
        requestFinishPrepareCourse(this.curCourse.courseId)
          ...
          .catch(err => {
            // console.log('requestFinishPrepareCourse FAIL', err)
          })
      },
      ...
      _getCourseResource: function(courseId) {
        this.loadingVisible = true
        sessionStorage.setItem('prepareLessonId', courseId)
    ...
    mounted: function() {
      ...
      if (courseId) {
        this._getCourseResource(courseId)
        return
      }
      // 缓存课程id
      const cacheCourseId = sessionStorage.getItem('prepareLessonId')
      if (cacheCourseId) {
        this._getCourseResource(cacheCourseId)
      }
      else {
        // 获取默认课程
